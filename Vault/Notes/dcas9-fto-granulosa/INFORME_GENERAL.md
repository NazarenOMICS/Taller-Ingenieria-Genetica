...
- **Guías múltiples independientes, dosis-respuesta y validación ortogonal** son recomendados como controles adicionales para atribución rigurosa (Omachi & Miner, 2021).
...
Corpus bueno (19 fuentes tras el ajuste). El eje **ROS/H₂O₂ → FTO↓ → m6A↑ → FOS↑ → senescencia** cuenta con evidencia causal en la **dirección de pérdida de función**, en líneas celulares de granulosa humana (Jiang et al., 2021c). Corresponde precisar el alcance de esa evidencia, según la auditoría textual completa de esa fuente registrada en `02_obligatorio/temas_candidatos/FTO_anexo_tecnico/11_auditoria_jiang_2021.md`: el trabajo es exclusivamente humano y exclusivamente in vitro o ex vivo, sin modelo animal ni experimentos in vivo; las líneas empleadas son COV434 y KGN; el fenotipo senescente no es replicativo ni espontáneo sino **inducido con peróxido de hidrógeno a 50 µM**; y los marcadores evaluados son γH2A.X y tinción de SA-β-galactosidasa, sin p16 ni p21. El rescate reportado consiste en que el silenciamiento de *FOS* alivia **parcialmente** el fenotipo provocado por la pérdida de FTO, y no en un experimento de ganancia de función de FTO sobre marcadores de senescencia. La señalización de Nrf2 actúa como puente crítico entre el estrés oxidativo y la regulación de m6A, promoviendo protección celular o permitiendo senescencia según su estado (Zhang et al., 2025 [helou]; Guo et al., 2022 [jun]; Xie et al., 2020; Shi et al., 2023). Existe evidencia específica de esta relación en granulosa humana, además de observaciones similares (menos detalladas) en osteoblastos y tejido cardíaco (Zhang et al., 2025; Zhao et al., 2021).
...
Corpus muy fuerte (23 fuentes). **Corrección aplicada el 2026-07-26 tras la auditoría textual de Jiang et al. (2021)**: este apartado sostenía que la sobreexpresión de FTO protege a la granulosa actuando como proteína retardadora de senescencia que revierte desórdenes epigenéticos y restaura la homeostasis celular, atribuyéndolo a esa fuente. Esa formulación excede lo que el trabajo demuestra y queda retirada.

Lo que Jiang et al. (2021) sostiene sobre ganancia de función es únicamente que, en las dos líneas que sobreexpresaron FTO, *FOS* resultó significativamente disminuido, dato que consta en la figura suplementaria S3. No hay en ese trabajo ningún experimento que mida marcadores de senescencia bajo aumento de FTO. Su afirmación de que FTO actúa como proteína retardadora de senescencia es una interpretación de los autores apoyada en la dirección del silenciamiento y en el rescate parcial por silenciamiento de *FOS*.

En cuanto al resto del apartado, FTO desestabiliza el ARNm de *FOS*, estabiliza *MMP2* activando la vía ERK (Li et al., 2024) y estabiliza *MIS12* contrarrestando la senescencia (Zhang et al., 2022b); los mecanismos se dividen en dependientes e independientes de m6A. Existen antecedentes de reversión de fenotipos de envejecimiento ovárico mediante intervenciones celulares y moleculares, entre ellos células madre mesenquimales de médula ósea que revierten el perfil de metilación m6A del ARN asociado a envejecimiento ovárico en granulosa envejecida (Tian et al., 2023).

**Regla de atribución adoptada.** Antes de asignar a cualquier otra fuente la demostración de reversión fenotípica por ganancia de función, debe verificarse que el trabajo incluya efectivamente aumento o restauración de FTO, células de granulosa o un modelo claramente comparable, medición de marcadores de senescencia, comparación experimental adecuada y evidencia causal de rescate. Los trabajos que demuestren ganancia de función en otro tejido, o únicamente disminución de *FOS*, se clasifican como **apoyo de plausibilidad** y no como demostración directa en granulosa. No se sustituye una cita incorrecta por otra evidencia indirecta. Las fuentes candidatas del corpus, entre ellas Wang et al. (2021a) sobre protección frente a cisplatino, Li et al. (2024) y Zhang et al. (2022b), quedan **pendientes de auditoría textual** antes de sostener esa afirmación.

Sobre **daño al ADN**: la afirmación de que la restauración de FTO disminuye marcadores de daño al ADN queda igualmente pendiente de auditoría de la fuente que la sostenga, y no se atribuye a Jiang et al. (2021).
...
Sobre C/EBPα, en el contexto del oncometabolito R-2HG en leucemia mieloide aguda se reporta que la supresión de CEBPA inhibe la transcripción de FTO; el pasaje anclado describe además la cascada FTO/m6A/MYC/CEBPA, en la que FTO opera aguas arriba y reduce la estabilidad de los transcritos de MYC y CEBPA, de modo que la direccionalidad no queda unívoca dentro de la misma fuente (Wang et al., 2020). La pregunta q16 aportó por separado evidencia de unión directa de C/EBPα al promotor de FTO, también en leucemia mieloide aguda y contextos metabólicos, no en granulosa. Sobre el receptor de andrógenos, su activación se asocia a un aumento de FTO en granulosa de pacientes con PCOS, en una retroalimentación positiva que agrava el hiperandrogenismo (Kuai et al., 2026). El punto central de este apartado es que ninguna fuente del corpus de q15 identifica un represor que se una al promotor de FTO; dentro de ese corpus el descenso se describe consistentemente como pérdida de activación y no como represión activa.

Corresponde matizar esa conclusión con evidencia externa al corpus, incorporada con posterioridad y verificada contra el texto completo de la fuente. Existe un elemento cis-regulatorio dentro del primer intrón de FTO, en la región asociada a adiposidad, reconocido por el factor de transcripción CUX1: el alelo protector en el SNP rs8050136 favorece la unión de la isoforma P110, que actúa como activador de la expresión de FTO y de RPGRIP1L, mientras que el alelo de riesgo es ocupado preferentemente por la isoforma P200, que actúa como represor transcripcional (Stratigopoulos et al., 2016). Ergo, sí existe un represor documentado sobre un elemento regulatorio de FTO, con dos salvedades que no conviene borrar: actúa sobre un elemento intrónico y no sobre el promotor proximal, y toda la evidencia proviene de contexto metabólico y neuronal, no de granulosa. El hallazgo tiene además una consecuencia de diseño, puesto que el mismo elemento regula a RPGRIP1L, que se ubica a menos de 100 pb corriente arriba de FTO y en orientación transcripcional opuesta (Stratigopoulos et al., 2016); el detalle y sus implicancias para la elección de la ventana blanco quedan registrados en la bitácora de decisiones de diseño del proyecto.
...
### 5.6 Justificación de dCas9-p300 como efector y consecuencia sobre la elección del editor

La elección de dCas9-p300 no depende de resolver primero el mecanismo promotor, y conviene exponer el argumento en orden afirmativo antes de llegar a la incertidumbre. En cuanto a la potencia, el sistema alcanza magnitudes de activación de 100 a 300 veces sobre el nivel basal en promotores endógenos humanos según el gen blanco, medidas en HEK293T sobre *NEUROD1* (cercano a 300 veces), *ASCL1* (entre 150 y 200 veces) y *MYOD1* (cercano a 100 veces), con activación superior a la de los sistemas basados en VP64 en el caso de *NEUROD1* (Omachi & Miner, 2022; Hilton et al., 2015). Ese rango es holgadamente suficiente para el objetivo de este proyecto, que no es sobreexpresar sino reponer un nivel fisiológico perdido, lo cual desplaza el problema desde la potencia hacia el control de dosis. En cuanto al mecanismo, p300 se diferencia de VP64 y VPR en que no recluta maquinaria transcripcional de forma transitoria sino que deposita H3K27ac en el locus dirigido, es decir, edita el estado epigenético (Hilton et al., 2015; Kaba et al., 2025); esa diferencia es la que vuelve al sistema conceptualmente coherente con una intervención sobre senescencia, donde el problema declarado es epigenético y no de dosis génica. En cuanto a la manejabilidad experimental, el trabajo fundacional documenta activación robusta con una sola guía cercana al TSS y una activación fuera de blanco transcriptómicamente limitada en HEK293T (Hilton et al., 2015), la marca depositada decae al retirar el efector sin memoria auto-sostenida (Park et al., 2016), lo cual convierte a la reversibilidad en un control experimental y no en una limitación, y existen estrategias de control ortogonal que permiten titular la activación dentro de un rango fisiológico en lugar de saturarla (Bonny et al., 2021). Cabe mencionar, para no presentar el argumento como incondicional, que la ventaja de p300 es dependiente del contexto celular: en fibroblastos primarios de piel humana adulta las fusiones dCas9-p300 no mejoraron los resultados de reprogramación frente a dCas9-VP192 (Adli, 2018), de modo que la superioridad observada en líneas inmortalizadas no puede darse por trasladada a granulosa sin medirla.

Los dos antecedentes que más peso aportan son de contexto y de blanco. Liao et al. (2026) dirigieron dCas9-p300 a la región P4 del promotor de *ZFP42* en células de la granulosa porcina y documentaron por ChIP-qPCR un aumento significativo del enriquecimiento de H3K27ac en esa región junto con un aumento del ARNm y de la proteína, lo cual constituye el precedente de tipo celular más cercano disponible para este proyecto. Kachanov et al. (2025), por su parte, activaron con el mismo sistema la expresión endógena de *FTO* en células HepG2 junto con otros componentes de la maquinaria m6A, obteniendo una reducción significativa del cccDNA del virus de la hepatitis B atribuible gen por gen; es el único precedente de activación del propio locus de *FTO* mediante dCas9-p300. Cabe aclarar dos precisiones de trazabilidad verificadas contra las fuentes primarias: en Liao et al. (2026) la dirección del efecto consta en el texto de resultados y no en el pasaje que el corpus del vault indexó, que corresponde a una leyenda de figura; y en Kachanov et al. (2025) la reducción medida en el brazo de activación es la de cccDNA, mientras que los datos de pgRNA provienen únicamente del brazo de pérdida de función, de modo que no corresponde atribuir ambos efectos a la sobreexpresión.

Sobre esa base, la incertidumbre mecanística descrita en los apartados anteriores no invierte la elección sino que la refuerza, aunque este último tramo debe leerse como inferencia de diseño y no como hecho demostrado. Si el mecanismo dominante fuese la pérdida de activación y no el silenciamiento activo por metilación, un editor desmetilante del tipo dCas9-TET1 carecería de sustrato que revertir, mientras que un escritor de acetilación opera por una vía compatible con reponer la activación perdida; y con independencia de cuál resulte ser la causa basal, p300 impone la marca activadora aguas abajo de todos los mecanismos candidatos. El contrapunto honesto es que, si el mecanismo real resultara ser una hipermetilación densa del promotor, la eficiencia de p300 podría quedar limitada por la accesibilidad de la cromatina, factor ya identificado como determinante en el apartado 3.1, y un editor combinado del tipo TET1 más p300 pasaría a ser preferible. Ergo, el mapeo del promotor de FTO en granulosa no es un experimento accesorio sino una decisión de diseño previa a la construcción del efector.
...
*Nota sobre el orden de ejecución:* el mapeo del promotor de *FTO* en granulosa (secuenciación por bisulfito y ChIP de H3K27ac sobre el locus, comparando granulosa joven contra envejecida o estresada oxidativamente) debe preceder al punto 1, porque de su resultado depende una decisión de diseño y no solo la interpretación. Si el promotor aparece con acetilación reducida y sin hipermetilación relevante, dCas9-p300 es el efector adecuado y el punto 1 procede tal como está descrito. Si en cambio aparece hipermetilación densa, la accesibilidad de la cromatina puede limitar la eficiencia del efector (apartado 3.1) y corresponde evaluar una estrategia combinada del tipo TET1 más p300 antes de gastar el ensayo de activación. Sin ese dato, un resultado negativo en el punto 1 sería ambiguo, puesto que no permitiría distinguir entre una guía mal posicionada, un efector insuficiente y una barrera de metilación no anticipada.
...
- **Liao et al., 2026** — Weili Liao; Nian Li; Yao Jiang; Enyuan Huang; Ke Mao; Bin Ma; Jianguo Zhao; Xiaolong Yuan (2026). *HDAC2-mediated H3K27ac governs ZFP42 transcription and autophagy in granulosa cells of pigs*. Life Sciences 401:124527. DOI: 10.1016/j.lfs.2026.124527.
...
- **Omachi & Miner, 2021** — Kohei Omachi; Jeffrey H. Miner (2021). *Comparative Analysis and Rational Design of dCas9-VP64 Variants for CRISPR Activation* (preprint). DOI: 10.1101/2021.08.13.456279.
- **Omachi & Miner, 2022** — Kohei Omachi; Jeffrey H. Miner (2022). *Comparative analysis of dCas9-VP64 variants and multiplexed guide RNAs mediating CRISPR activation*. DOI: 10.1371/journal.pone.0270008.
...
- **Stratigopoulos et al., 2011** — George Stratigopoulos et al. (2011). *Cut-like homeobox 1 (CUX1) regulates expression of the fat mass and obesity-associated and retinitis pigmentosa GTPase regulator-interacting protein-1-like (RPGRIP1L) genes and coordinates leptin receptor signaling*. Journal of Biological Chemistry 286(3):2155-2170. DOI: 10.1074/jbc.M110.188482. PMID: 21037323.
- **Stratigopoulos et al., 2014** — George Stratigopoulos et al. (2014). *Hypomorphism for RPGRIP1L, a ciliary gene vicinal to the FTO locus, causes increased adiposity in mice*. Cell Metabolism 19(5):767-779. DOI: 10.1016/j.cmet.2014.04.009. PMID: 24807221.
- **Stratigopoulos et al., 2016** — George Stratigopoulos et al. (2016). *Hypomorphism of Fto and Rpgrip1l causes obesity in mice*. Journal of Clinical Investigation. DOI: 10.1172/JCI85526. PMID: 27064284.
